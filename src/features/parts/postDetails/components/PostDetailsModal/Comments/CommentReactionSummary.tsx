...
export function CommentReactionSummary({
  ...
  const summaryLabel = total === 1 ? "1 reaction" : `${total} reactions`;
  ...
  const baseProps = {
    "aria-label": `View all ${summaryLabel}`,
    "data-testid": "comment-reaction-summary",
  } as const;
  ...
  return (
    <div
      className="flex items-center gap-2 rounded-full bg-secondary/40 px-2 py-0.5 text-xs font-semibold text-muted-foreground"
      role="status"
      aria-live="polite"
      {...baseProps}
    >
