...
const { removeFollower } = jest.requireMock(
  "@/features/parts/follow/services/server"
) as { removeFollower: jest.Mock };
...
describe("/api/follow/[username]/remove-follower DELETE", () => {
  beforeEach(() => {
    jest.clearAllMocks();
  });
  ...
  it("short-circuits when preparation fails", async () => {
    ...
    const response = await serverFactory()
      .delete("/api/follow/target-user/remove-follower")
      .send({});
  ...
  it("removes follower and updates relationship snapshot", async () => {
    ...
    removeFollower.mockResolvedValue({ status: "REMOVED" });
    ...
    expect(response.body).toEqual({
      success: true,
      data: { status: "REMOVED" },
      message: followMessages.FEEDBACK.removeFollowerSuccess,
    });
  });

  it("returns normalized error when removeFollower fails", async () => {
    const log = createMockLog();
    const context = {
      requestId: "req-500",
      log,
    ...
    prepareFollowAction.mockResolvedValue({ ok: true, context });
    removeFollower.mockRejectedValue(serviceError);
    ...
    expect(log.error).toHaveBeenCalledWith(
      ...
      "Remove follower request failed"
    );
    expect(adjustRelationshipSnapshot).not.toHaveBeenCalled();
